...
//
#line 548 "C:\Users\elektromeier\Desktop\VomAmt\VomAmtVR\lib\log\shader_VLEffectMain_1ba67c1df8a6505795fe8a07bf0e0836.hlsl"
...
@G    Pixel => ba94376eadd0d1b1ddeacb3c797d9410
//
// Generated by Microsoft (R) HLSL Shader Compiler 10.1
//
//
...
//
#line 424 "C:\Users\elektromeier\Desktop\VomAmt\VomAmtVR\lib\log\shader_VLEffectMain_1ba67c1df8a6505795fe8a07bf0e0836.hlsl"
mad r0.x, cb0[1].z, l(2.000000), l(-1.000000)
max r0.x, r0.x, l(0.000000)  // r0.x <- diffuseFactor

#line 259
add r0.yz, v3.xxyx, cb0[0].xxyx  // r0.y <- <Compute_id71 return value>.x; r0.z <- <Compute_id71 return value>.y

#line 243
mad r0.yz, r0.yyzy, cb0[0].zzwz, cb0[1].xxyx  // r0.y <- <Compute_id87 return value>.x; r0.z <- <Compute_id87 return value>.y

#line 239
sample_indexable(texture2d)(float,float,float,float) r1.xyzw, r0.yzyy, t0.xyzw, s0  // r1.x <- <Compute_id91 return value>.x; r1.y <- <Compute_id91 return value>.y; r1.z <- <Compute_id91 return value>.z; r1.w <- <Compute_id91 return value>.w
...
float2 Compute_id84(inout PS_STREAMS streams)
{
...
float2 Compute_id76(inout PS_STREAMS streams)
{
...
float2 Compute_id68(inout PS_STREAMS streams)
{
...
void Compute_id129(inout PS_STREAMS streams)
{
    streams.Input_10_id181 = Compute_id126();
}
float4 Compute_id91(inout PS_STREAMS streams)
{
    return DynamicTexture_id166.Sample(DynamicSampler_id167, Compute_id88(streams));
}
float2 Compute_id87(inout PS_STREAMS streams)
{
    return Compute_id82(streams) + Compute_id84(streams);
}
float2 Compute_id80()
{
...
{
    return Compute_id74(streams) * Compute_id76(streams);
}
float2 Compute_id72()
{
...
{
    return Compute_id66(streams) + Compute_id68(streams);
}
float2 Compute_id64()
{
...
float Compute_id132(inout PS_STREAMS streams)
{
    return streams.Input_10_id181;
}
void Compute_id131(inout PS_STREAMS streams)
{
...
{
    streams.TextureValue_9_id165 = Compute_id91(streams);
}
void Compute_id106(inout PS_STREAMS streams)
{
    streams.Plus_8_id161 = Compute_id87(streams);
}
void Compute_id104(inout PS_STREAMS streams)
{
    streams.Input_7_id158 = Compute_id80();
}
void Compute_id102(inout PS_STREAMS streams)
{
    streams.Multiply_6_id154 = Compute_id79(streams);
}
void Compute_id100(inout PS_STREAMS streams)
{
    streams.Input_5_id151 = Compute_id72();
}
void Compute_id98(inout PS_STREAMS streams)
{
    streams.Plus_3_id146 = Compute_id71(streams);
}
void Compute_id96(inout PS_STREAMS streams)
{
    streams.Input_2_id143 = Compute_id64();
}
void Compute_id94()
{
}
...
float Compute_id135(inout PS_STREAMS streams)
{
    Compute_id131(streams);
    return Compute_id132(streams);
}
float Compute_id120()
{
    return Value_id176;
...
void Compute_id110(inout PS_STREAMS streams)
{
...
{
    Compute_id145();
    return Compute_id146();
}
float4 Compute_id137(inout PS_STREAMS streams)
{
    float x = Compute_id135(streams);
    return float4(x, x, x, x);
}
float Compute_id123()
{
    Compute_id119();
    return Compute_id120();
}
float4 Compute_id114(inout PS_STREAMS streams)
{
    Compute_id110(streams);
    return Compute_id111(streams);
}
void ResetStream_id61()
{
}
float4 Compute_id151()
{
    return Compute_id149();
}
float4 Compute_id142(inout PS_STREAMS streams)
{
    float alpha = 2.0 * Compute_id137(streams).x;
    float specularFactor = min(1, alpha);
    float diffuseFactor = max(0, alpha - 1.0);
    return float4(diffuseFactor, specularFactor, 0, 0);
}
float4 Compute_id125()
{
    float x = Compute_id123();
    return float4(x, x, x, x);
}
float4 Compute_id116(inout PS_STREAMS streams)
{
    return Compute_id114(streams);
}
...
void Compute_id171(inout PS_STREAMS streams)
{
    streams.matAlphaBlendColor_id65 = Compute_id151().rgb;
}
void Compute_id166(inout PS_STREAMS streams)
{
    streams.matDiffuseSpecularAlphaBlend_id64 = Compute_id142(streams).rg;
}
void Compute_id161(inout PS_STREAMS streams)
{
    streams.matEmissiveIntensity_id62 = Compute_id125().r;
}
void Compute_id156(inout PS_STREAMS streams)
{
    streams.matEmissive_id61 = Compute_id116(streams).rgba;
}
